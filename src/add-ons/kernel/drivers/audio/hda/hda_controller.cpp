	...
	uint32 index)
{
	...
	if (stream->use_dma_position && stream->incorrect_position_count >= 32) {
		dprintf("hda: DMA position for stream (id:%" B_PRIu32 ") seems to be broken. "
			...
			if ((rirbStatus & RIRB_STATUS_RESPONSE) != 0) {
				...
				for (; controller->rirb_read_pos != writePos;
					...
					if ((responseFlags & RESPONSE_FLAGS_UNSOLICITED) != 0) {
						dprintf("hda: Unsolicited response: %08" B_PRIx32 "/%08" B_PRIx32 "\n",
					...
					if (codec->response_count >= MAX_CODEC_RESPONSES) {
						dprintf("hda: too many responses received for codec %" B_PRIu32
							": %08" B_PRIx32 "/%08" B_PRIx32 "!\n", cad, response, responseFlags);
						continue;
					}
	...
	const char* desc)
{
	...
	dprintf("hda: sample size %" B_PRIu32 ", num channels %" B_PRIu32 ", buffer length %" B_PRIu32 "\n",
		stream->sample_size, stream->num_channels, stream->buffer_length);
	dprintf("hda: %s: setup stream %" B_PRIu32 ": SR=%" B_PRIu32 ", SF=%" B_PRIu32 " F=0x%x (0x%" B_PRIu32 ")\n",
	...
	dprintf("hda: %s(%s): Allocated %" B_PRIu32 " bytes for %" B_PRIu32 " buffers\n", __func__, desc,
	...
	dprintf("hda: %s(%s): Allocated %" B_PRIu32 " bytes for %" B_PRIu32 " BDLEs\n", __func__, desc,
		alloc, bdlCount);
	...
	dprintf("hda: stream: %" B_PRIu32 " fifo size: %d num_io_widgets: %" B_PRIu32 "\n", stream->id,
...
hda_hw_init(hda_controller* controller)
{
	...
	dprintf("hda: HDA v%d.%d, O:%" B_PRIu32 "/I:%" B_PRIu32 "/B:%" B_PRIu32 ", #SDO:%" B_PRIu32 ", 64bit:%s\n",
