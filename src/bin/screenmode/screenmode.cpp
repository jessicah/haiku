...
print_mode(const display_mode& displayMode, const screen_mode& mode)
{
	...
	printf("%" B_PRIu32 "  %u %u %u %u  %u %u %u %u ",
		timing.pixel_clock / 1000, timing.h_display, timing.h_sync_start,
...
main(int argc, char** argv)
{
	...
	if (status == B_OK) {
		...
		fprintf(stderr, "%s: Could not set screen mode %" B_PRId32
			"x%" B_PRId32 "x%" B_PRId32 ": %s\n",
